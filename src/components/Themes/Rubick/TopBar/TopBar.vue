...
<template>
  ...
  <div class="relative z-[51] flex h-[67px] items-center border-b border-slate-200">
    ...
    <Breadcrumb class="hidden mr-auto -intro-x sm:flex">
      <Breadcrumb.Link to="/">{{ $t('admin') }}</Breadcrumb.Link>
      <Breadcrumb.Link to="/" :active="true"> {{ $t('dashboard') }} </Breadcrumb.Link>
    </Breadcrumb>
    <!-- END: Breadcrumb -->
    <!-- BEGIN: Search -->
    ...
    <!-- <Popover class="mr-auto intro-x sm:mr-6">
      ...
      <Popover.Panel class="w-[280px] sm:w-[350px] p-5 mt-2">
        <div class="mb-5 font-medium">Reports</div>
    ...
    <Menu>
      <div class="flex items-center gap-4 ml-auto mr-4">
        ...
        <!-- Til tanlash dumaloq menyusi -->
        <Menu as="div" class="relative">
          <Menu.Button
            class="w-8 h-8 flex items-center justify-center rounded-full bg-primary text-white text-xs font-bold shadow-md intro-x"
            title="Tilni tanlash">
            {{ locale.toUpperCase() }}
          </Menu.Button>
          <Menu.Items
            class="absolute sm:right-0 mt-2 w-24 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
            <Menu.Item v-for="lang in ['uz', 'ru', 'en']" :key="lang" v-slot="{ active }">
              <button @click="locale = lang" class="w-full px-3 py-2 text-sm text-left"
                :class="active ? 'bg-slate-100 text-primary font-medium' : 'text-slate-800'">
                {{ lang.toUpperCase() }}
              </button>
            </Menu.Item>
          </Menu.Items>
        ...
        <Menu.Items class="w-56 mt-px text-white bg-primary">
          ...
          <Menu.Divider class="bg-white/[0.08]" />
          <Menu.Item @click="logout" class="hover:bg-white/5 cursor-pointer">
